...
// Generate RSA-2048 key pair on startup (WebCrypto API, available in Bun).
...
function b64url(data: BufferSource | string): string {
  const bytes = typeof data === 'string' ? new TextEncoder().encode(data) : new Uint8Array(data instanceof ArrayBuffer ? data : (data as ArrayBufferView).buffer)
  let b64 = ''
...
console.log(`Mock OIDC server running — issuer: ${ISSUER}`)
